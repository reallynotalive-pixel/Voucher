...
async def init_db():
“”“Create tables (vouches + guild_config) with all columns.”””
async with _db_lock, db_open(DB_FILE) as db:
# Main vouches table — includes v2 columns vouch_type & verified
await db.execute(”””
...
```
    # Safely add new columns to existing DBs (idempotent)
    for col, definition in [
        ("vouch_type", "TEXT NOT NULL DEFAULT 'TRADE'"),
        ("verified",   "INTEGER NOT NULL DEFAULT 0"),
    ]:
        try:
            await db.execute(f"ALTER TABLE vouches ADD COLUMN {col} {definition}")
        except Exception:
...
_softlock_previous: dict[int, discord.PermissionOverwrite] = {}
...
async with _db_lock, db_open(DB_FILE) as db:
# Same voucher → same target within 7 days
cur = await db.execute(
“””
...
trusted_role    = guild.get_role(int(trusted_role_id))    if trusted_role_id    and int(trusted_role_id)    != 0 else None
restricted_role = guild.get_role(int(restricted_role_id)) if restricted_role_id and int(restricted_role_id) != 0 else None
...
should_trust    = (total >= int(trusted_min_vouches or 0)    and avg >= float(trusted_min_avg or 0))
should_restrict = (total >= int(restricted_min_vouches or 0) and avg <= float(restricted_max_avg or 0))
...
try:
    if restricted_role and can_manage(restricted_role):
        if should_restrict and restricted_role not in member.roles:
            ...
            await member.remove_roles(restricted_role, reason="Trust gate: remove restricted")
    ...
    if trusted_role and can_manage(trusted_role):
        if should_trust and trusted_role not in member.roles:
            ...
            await member.remove_roles(trusted_role, reason="Trust gate: remove trusted")
...
for i, row in enumerate(rows, start=1):
    # Unpack — support both old (8-col) and new (10-col) rows gracefully
    ...
    value = (
        ...
        f"**From:** <@{voucher_id}>\n"
        f"**Trader:** <@{trader_id}>\n"
        f"**Middleman:** {f'<@{middleman_id}>' if middleman_id else 'None'}\n"
        f"**Rating:** {stars}{flag}\n"
        f"**Item:** {traded_item}\n"
        f"**Date:** {created_at}\n"
...
def **init**(self, *, requester_id: int, guild_id: int, target_member: discord.Member):
...
async def on_submit(self, interaction: discord.Interaction):
    ...
    try:
        ...
        return await interaction.response.send_message(
            f"{CROSS} Rating must be a number between **1 and 5**.", ephemeral=True
        )
    ...
    async with _db_lock, db_open(DB_FILE) as db:
        cursor = await db.execute(
            ...
            (
                ...
                suspicious,
                self.vouch_type,
                0,  # unverified until vouched user confirms
            ),
        )
        await db.commit()
    ...
    embed.add_field(name="Middleman",   value=self.middleman.mention if self.middleman else "None", inline=False)
    ...
    embed.set_footer(
        text=f"Vouch submitted by {interaction.user}  •  ID: {vouch_id}",
    ...
    try:
        ...
        dm_embed.add_field(name="From",       value=interaction.user.mention,                          inline=False)
        dm_embed.add_field(name="Type",        value=self.vouch_type,                                   inline=True)
        dm_embed.add_field(name="Rating",      value=STAR * stars,                                      inline=True)
        dm_embed.add_field(name="Trader",      value=self.trader.mention,                               inline=False)
        dm_embed.add_field(name="Middleman",   value=self.middleman.mention if self.middleman else "None", inline=False)
        dm_embed.add_field(name="Item",        value=self.traded_item.value,                            inline=False)
        if suspicious:
            dm_embed.add_field(
                name="Notice",
        ...
        dm_embed.set_footer(text="Press Confirm to verify, or Deny to remove this vouch.")

        await self.vouched_user.send(
            embed=dm_embed,
            view=VerifyVouchView(vouch_id=vouch_id, vouched_user_id=self.vouched_user.id),
            allowed_mentions=discord.AllowedMentions(users=True),
        )
    except Exception:
        pass  # DMs may be disabled — that's okay
...
tv, ta = state.trusted_min_vouches, state.trusted_min_avg
rv, ra = state.restricted_min_vouches, state.restricted_max_avg
embed.add_field(
    name="Thresholds",
    value=(
...
async def interaction_check(self, interaction: discord.Interaction) -> bool:
    if interaction.user.id != self.requester_id:
        await interaction.response.send_message(
            f"{CROSS} Only the admin who started setup can use this.", ephemeral=True
        )
        return False
    return True
...
async def _on_channel_selected(self, interaction: discord.Interaction):
    self.state.status_channel_id = (
        self.channel_select.values[0].id if self.channel_select.values else 0
    )
    await self._refresh(interaction)

async def _on_trusted_selected(self, interaction: discord.Interaction):
    self.state.trusted_role_id = (
        self.trusted_role_select.values[0].id if self.trusted_role_select.values else 0
    )
    await self._refresh(interaction)

async def _on_restricted_selected(self, interaction: discord.Interaction):
    self.state.restricted_role_id = (
        self.restricted_role_select.values[0].id if self.restricted_role_select.values else 0
    )
    await self._refresh(interaction)
...
async def save_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    # Fill in any untouched fields from existing config
    ...
    embed.description = f"{CHECK} Saved! Run `/setup` to view or `/setupwizard` to change again."
    await interaction.response.edit_message(embed=embed, view=self)
...
@bot.tree.command(name=“vouchbuy”, description=“Create a BUY vouch (you bought from someone)”)
...
@bot.tree.command(name=“vouchsell”, description=“Create a SELL vouch (you sold to someone)”)
...
@bot.tree.command(name=“vouchtrade”, description=“Create a TRADE vouch (item-for-item swap)”)
...
embed.add_field(name="MM avg",           value=(f"{mm_avg:.2f}/5" if mm_avg is not None else "N/A"), inline=True)
...
embed.description = "\n".join(lines)
await interaction.response.send_message(
    embed=embed, ephemeral=True, allowed_mentions=discord.AllowedMentions(users=True)
)
```
...
@bot.tree.command(name=“maxmute”, description=“Timeout a user for the maximum duration (Admin only).”)
...
if me is None or not me.guild_permissions.moderate_members:
    return await interaction.response.send_message(
        f"{CROSS} I need the **Moderate Members** permission to timeout users.", ephemeral=True
    )
...
if me is None or not me.guild_permissions.moderate_members:
    return await interaction.response.send_message(
        f"{CROSS} I need the **Moderate Members** permission to unmute users.", ephemeral=True
    )
...
await interaction.response.send_message(f"{CHECK} Soft-unlocked {channel.mention}.")
```
...
@bot.tree.command(name=“setup”, description=“Show current setup/config values for this server.”)
...
protected_lines = [f"<@&{rid}> (`{rid}`)" for rid in protected_ids] if protected_ids else ["None"]
...
embed.add_field(name="Status Channel", value=_fmt_channel(int(status_channel_id) if status_channel_id else 0), inline=False)
embed.add_field(name="Trusted Role",   value=_fmt_role(int(trusted_role_id) if trusted_role_id else 0),    inline=False)
embed.add_field(name="Restricted Role",value=_fmt_role(int(restricted_role_id) if restricted_role_id else 0), inline=False)
embed.add_field(name="Protected Roles", value="\n".join(protected_lines)[:1024],                           inline=False)
embed.add_field(
    name="Thresholds",
    value=(
...
config_group = app_commands.Group(name=“config”, description=“Change Voucher Bot settings (Admin only).”)
...
@config_group.command(name=“reset_defaults”, description=“Reset this server back to file defaults.”)
async def reset_defaults(interaction: discord.Interaction):
if not _admin_only(interaction):
return await interaction.response.send_message(f”{CROSS} Admin only.”, ephemeral=True)
async with _db_lock, db_open(DB_FILE) as db:
await db.execute(“DELETE FROM guild_config WHERE guild_id=?”, (interaction.guild_id,))
await db.commit()
await interaction.response.send_message(
f”{CHECK} Reset complete. Run `/setup` to confirm.”, ephemeral=True
)

@config_group.command(name=“set_owner”, description=“Set the owner ID used for /shutdown authorization.”)
...
return await interaction.response.send_message(f”{CROSS} Must be a number.”, ephemeral=True)
await cfg_set(interaction.guild_id, “OWNER_ID”, oid)
await interaction.response.send_message(f”{CHECK} Owner ID set to `{oid}`.”, ephemeral=True)

@config_group.command(name=“set_status_channel”, description=“Set the status channel for online/offline notices.”)
...
@config_group.command(name=“disable_trusted_role”, description=“Disable Trusted auto-role assignment.”)
async def disable_trusted_role(interaction: discord.Interaction):
if not _admin_only(interaction):
return await interaction.response.send_message(f”{CROSS} Admin only.”, ephemeral=True)
await cfg_set(interaction.guild_id, “TRUSTED_ROLE_ID”, 0)
await interaction.response.send_message(f”{CHECK} Trusted role disabled.”, ephemeral=True)

@config_group.command(name=“set_restricted_role”, description=“Set the Restricted role.”)
@app_commands.describe(role=“Pick a role”)
...
@config_group.command(name=“disable_restricted_role”, description=“Disable Restricted auto-role assignment.”)
async def disable_restricted_role(interaction: discord.Interaction):
if not _admin_only(interaction):
return await interaction.response.send_message(f”{CROSS} Admin only.”, ephemeral=True)
await cfg_set(interaction.guild_id, “RESTRICTED_ROLE_ID”, 0)
await interaction.response.send_message(f”{CHECK} Restricted role disabled.”, ephemeral=True)

@config_group.command(name=“add_protected_role”, description=“Add a role to staff immunity (protected).”)
...
@config_group.command(name=“remove_protected_role”, description=“Remove a role from staff immunity.”)
...
@config_group.command(name=“set_thresholds”, description=“Set trust gate thresholds.”)
...
# Notify each guild's status channel
for g in bot.guilds:
    try:
        status_id = await cfg_get(g.id, "STATUS_CHANNEL_ID")
        if not status_id:
        ...
        if ch:
            await ch.send("**AMP VOUCHER BOT CURRENTLY OFFLINE AND UNDER MAINTENANCE**")
    except Exception:
        pass
