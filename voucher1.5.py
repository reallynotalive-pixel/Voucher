...
async def cfg_get(guild_id: int, key: str):
    """Get config value for guild, falling back to DEFAULT_CONFIG."""
    async with _db_lock, db_open(DB_FILE) as db:
...
async def cfg_set(guild_id: int, key: str, value):
    """Set config value for guild (stored as JSON)."""
...
async def apply_trust_gate(guild: discord.Guild, member: discord.Member):
    ...
    if (not trusted_role_id or int(trusted_role_id) == 0) and (not restricted_role_id or int(restricted_role_id) == 0):
        return
...
async def on_ready():
    ...
    if not _sent_online:
        _sent_online = True
        # Send an online notice to each server's configured status channel (if set).
        for g in bot.guilds:
            try:
...
async def trust(interaction: discord.Interaction, user: discord.Member):
    ...
    trusted_min_avg = await cfg_get(interaction.guild_id, "TRUSTED_MIN_AVG") or 4.5
    if total >= int(trusted_min_vouches) and (avg or 0) >= float(trusted_min_avg):
...
async def setup(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description="Use `/config` to change these values (saved in the DB).",
        color=discord.Color.blurple(),
    )
    ...
    embed.add_field(name="Protected Roles (Immunity)", value="\n".join(protected_lines)[:1024], inline=False)
    embed.add_field(
        name="Thresholds",
        value=(
...
async def reset_defaults(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"{CHECK} Reset complete. Run `/setup` to confirm.", ephemeral=True)
...
@config_group.command(name="set_status_channel", description="Set the status channel used for online/offline notices.")
...
async def set_status_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    await interaction.response.send_message(f"{CHECK} Status channel set to {channel.mention}.", ephemeral=True)
...
async def set_trusted_role(interaction: discord.Interaction, role: discord.Role):
    ...
    await interaction.response.send_message(f"{CHECK} Trusted role set to {role.mention}.", ephemeral=True)
...
async def set_restricted_role(interaction: discord.Interaction, role: discord.Role):
    ...
    await interaction.response.send_message(f"{CHECK} Restricted role set to {role.mention}.", ephemeral=True)
...
async def add_protected_role(interaction: discord.Interaction, role: discord.Role):
    ...
    await interaction.response.send_message(f"{CHECK} Added protected role: {role.mention}", ephemeral=True)

@config_group.command(name="remove_protected_role", description="Remove a role from staff immunity (protected).")
...
async def remove_protected_role(interaction: discord.Interaction, role: discord.Role):
    ...
    await interaction.response.send_message(f"{CHECK} Removed protected role: {role.mention}", ephemeral=True)
...
@app_commands.describe(
    ...
    restricted_max_avg="Restricted max average rating (or lower triggers restricted)",
)
async def set_thresholds(
    ...
    await interaction.response.send_message(f"{CHECK} Thresholds updated. Run `/setup` to view.", ephemeral=True)
...
def _install_signal_handlers():
    ...
    async def _graceful_close():
        ...
        # send offline notice to each guild's configured status channel
        for g in bot.guilds:
            try:
                status_id = await cfg_get(g.id, "STATUS_CHANNEL_ID")
                if not status_id:
...
async def shutdown(interaction: discord.Interaction, code: str):
    owner_id = await cfg_get(interaction.guild_id, "OWNER_ID") if interaction.guild_id else OWNER_ID
